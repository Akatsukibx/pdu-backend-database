...
export default function AddPduView({ onCreated, onCancel }) {
  ...
  const submit = async (e) => {
    ...
    // ถ้ายังไม่ได้ทำ SNMPv3 ใน backend จริง ๆ ให้ล็อกก่อน
    ...
    try {
      const payload = {
        ...
        snmp_timeout_ms: Number(form.snmp_timeout_ms || 2000),
        snmp_retries: Number(form.snmp_retries || 1),
      ...
      setForm((prev) => ({
        ...DEFAULT_FORM,
        // เก็บค่าบางอย่างไว้ให้กรอกตัวถัดไปง่ายขึ้น
        brand: payload.brand,
        model: payload.model,
  ...
  <label className="pdu-checkbox">
    ...
    ใช้งาน (ให้ poller ดึง)
        ...
        <div className="form-group">
          ...
          <input
            type="number"
            value={form.snmp_timeout_ms}
            onChange={(e) => set("snmp_timeout_ms", e.target.value)}
          />
        </div> */}
        ...
        <div className="form-group">
          ...
          <input
            type="number"
            value={form.snmp_retries}
            onChange={(e) => set("snmp_retries", e.target.value)}
          />
        </div> */}

        {/* spacer ให้กริด 2 คอลัมน์บาลานซ์ */}
        <div className="form-group" />
